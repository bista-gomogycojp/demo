...
export default {
    async actLogin({dispatch}, {user_name, password}) {
        try {
            ...
            if (response.status === 200 && response.data) {
                localStorage.setItem('access_token', JSON.stringify(response.data.tokens) )
    ...
    async actAddLogoForUser({dispatch,state},{ logo_box }){
        try {
            const token = state.token;
            const id = state.currentUser.id
            const user_name = state.currentUser.user_name
            if(token && id){
                const config = {
                    headers: {
                        Authorization: `JWT ${token}`,
                ...
                const response =  await axiosInstance.put(`/user/upLogo/${id}/`,data,config);
    ...
    async actAddDarkForUser({dispatch,state, commit},{ dark }){
        try {
            const token = state.token;
            const id = state.currentUser.id
            const user_name = state.currentUser.user_name
            if(token && id){
                ...
                const response = await axiosInstance.put('/user/upDark/' + id + '/',data,config);
                if(response.status === 200){
                    return {
                        status : true
                    }
    ...
    async actAddCategoryForUser({dispatch,state, commit},{ category }){
        try {
            const token = state.token;
            const id = state.currentUser.id
            const user_name = state.currentUser.user_name
            if(token && id){
                ...
                const response = await axiosInstance.put('/user/upCategory/' + id + '/',data,config);
                if(response.status === 200){
                    return {
                        data: response.data,
                        status : true
    ...
    async actAddCatItemForUser({dispatch,state,commit},{ category_item }){
        try {
            const token = state.token;
            const id = state.currentUser.id
            const user_name = state.currentUser.user_name
            if(token && id){
                ...
                const response = await axiosInstance.put('/user/upCategoryItem/' + id + '/',data,config);
                if(response.status === 200){
                    return {
                        data: response.data,
                        status : true
    ...
    async actAddPageForUser({dispatch,state,commit},{ pagination }){
        try {
            const token = state.token;
            const id = state.currentUser.id
            const user_name = state.currentUser.user_name
            if(token && id){
                ...
                const response = await axiosInstance.put('/user/upPagination/' + id + '/',data,config);
