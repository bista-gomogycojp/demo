...
async function setHandleSubmit(isLoading, autoSave , password, confirm_password, notification,
                               username, email, validateEmail, validatePassword,
                               store, message, level, times, code,router
) {
    isLoading = true
    ...
    if (username.value && password.value || email.value || code.value) {
        ...
        if (level === 2) {
            ...
            if(autoSave.value){
                localStorage.setItem('saveAccount', JSON.stringify(data) )
                localStorage.setItem('autoSaveBtn', JSON.stringify("save") )
            } else {
                localStorage.removeItem('saveAccount')
                localStorage.removeItem('autoSaveBtn')
            }
        ...
        if (level === 3) {
            if(email.value){
                ...
                notification_error(notification, 'ご登録されているメールアドレスを入力してください。')
            }
        }
        if (level === 4) {
            if (password.value !== confirm_password.value) {
